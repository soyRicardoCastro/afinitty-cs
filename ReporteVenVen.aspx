<%@ Page Title="Tarrago Reach System" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ReporteVenVen.aspx.cs" Inherits="ReachSystem.ReporteVenVen" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server"> 
    ...
    <div class="card">
        <div class="card-header">
                <div class="form-row">
                    <div class="col10L">                        
                        <asp:Label ID="Label6" runat="server" Text="REPORTE: GENERAL DE VENTAS" Style="font-weight: bold; font-size: 18px;"></asp:Label>                        
                    </div>                   
                </div>
            </div>
        <div class="card-body">
                <div class="form-row">
                    <table style="width: 100%;">
                        <tr>  
                            ...
                            <td><asp:Label ID="Label4" runat="server" Text="EJECUTIVO" Style="font-weight: bold; font-size: 18px;"></asp:Label><br />
                                <asp:DropDownList ID="DropDownList1" runat="server" class="formularioProc" DataSourceID="SqlDataSource2" DataTextField="Alias" DataValueField="Alias"></asp:DropDownList>
                                <asp:SqlDataSource runat="server" ID="SqlDataSource2" ConnectionString='<%$ ConnectionStrings:affisDBConnectionString %>' SelectCommand="SELECT Alias FROM Usuario Where Perfil = 'VENTA' OR Perfil = 'SUPERVEN' OR Perfil = 'DIRECVEN' OR Perfil = 'SERVICIO'"></asp:SqlDataSource>
                            </td>
                    ...
                    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" AllowPaging="True" GridLines="None" BackColor="White" BorderStyle="None" BorderWidth="1px" CellPadding="20" ForeColor="Black" CssClass="mGrid" PagerStyle-CssClass="pgr" margin-left="2%" AlternatingRowStyle-CssClass="alt" AllowSorting="True" DataSourceID="SqlDataSource1" PageSize="100">
                        <Columns>
                            ...
                            <asp:BoundField DataField="Fecha" HeaderText="FECHA VENTA" SortExpression="Fecha"></asp:BoundField>                            
                            <asp:BoundField DataField="NombreComercial" HeaderText="EMPRESA" SortExpression="NombreComercial"></asp:BoundField>
                            <asp:BoundField DataField="Certificados" HeaderText="CANTIDAD CERTIFICADOS" SortExpression="Certificados"></asp:BoundField>                            
                            <asp:BoundField DataField="Tomadores" HeaderText="CANTIDAD TOMADORES" SortExpression="Tomadores"></asp:BoundField>
                            <asp:BoundField DataField="Primas" HeaderText="SUB TOTALES PRIMAS" SortExpression="Primas"></asp:BoundField>
                    ...
                    <asp:SqlDataSource runat="server" ID="SqlDataSource1" ConnectionString='<%$ ConnectionStrings:affisDBConnectionString %>' SelectCommand="SELECT Ejecutivo, CAST(FechaVenta AS DATE) as Fecha, NombreComercial, COUNT(NOMBRE) as Certificados, COUNT(DISTINCT NOMBRE) as Tomadores, SUM(TotalCobrar) as Primas FROM Asegurados WHERE ([NumeroCertificado] IS NOT NULL) AND fechaventa BETWEEN @fechaini AND @fechafin AND Ejecutivo = @est group by NombreComercial, CAST(FechaVenta AS DATE), Ejecutivo">
                        <SelectParameters>                                
